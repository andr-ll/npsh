...
function svc() {
  ...
  const PACKAGE_JSON_URL = `https://raw.githubusercontent.com/${user}/${project}/${branch}/package.json`;

  const semanticVersions = ['major', 'minor', 'patch'];

  https.get(PACKAGE_JSON_URL, async (res) => {
    let rawData = '';
    ...
    res.on('end', () => {
      try {
        ...
        const masterVersionArr = version
          .split('.')
